...
if (!empty($busca)) {
    $where = "WHERE nome LIKE :busca OR email LIKE :busca OR cargo LIKE :busca";
    $params[':busca'] = '%' . $busca . '%';
}

$sql = "SELECT * FROM candidatos $where ORDER BY data_cadastro DESC";
            ...
            <div class="table-container">
                <div class="table-header">
                    ...
                    <div style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
                        <form method="GET" action="dashboard.php" style="display: flex; gap: 0.5rem;">
                            <input type="text" name="busca" placeholder="Buscar candidato..." value="<?= htmlspecialchars($_GET['busca'] ?? '') ?>" class="search-input" style="padding: 0.5rem 1rem; border: 2px solid var(--border-color); border-radius: 0.5rem; font-family: 'Poppins', sans-serif;">
                            <button type="submit" class="btn btn-primary btn-icon" title="Buscar">
                                <i class="fas fa-search"></i>
                            </button>
                            <?php if (isset($_GET['busca']) && $_GET['busca'] != ''): ?>
                                <a href="dashboard.php" class="btn btn-secondary btn-icon" title="Limpar busca">
                ...
                <?php if (empty($candidatos)): ?>
                    <div class="empty-state">
                        ...
                        <h3><?= !empty($busca) ? 'Nenhum candidato encontrado' : 'Nenhum candidato cadastrado' ?></h3>
                        <p><?= !empty($busca) ? 'Tente buscar com outros termos' : 'Comece cadastrando seu primeiro candidato' ?></p>
                        <?php if (empty($busca)): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <a href="ver_candidato.php?id=<?= $candidato['id'] ?>" class="btn btn-small btn-success btn-icon" title="Visualizar">
