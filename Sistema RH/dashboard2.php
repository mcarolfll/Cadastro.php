...
if (!empty($busca)) {
    $where = "WHERE nome LIKE :busca OR email LIKE :busca OR cnpj LIKE :busca";
    $params[':busca'] = '%' . $busca . '%';
}

$sql = "SELECT * FROM empresas $where ORDER BY data_cadastro DESC";
            ...
            <div class="table-container">
                <div class="table-header">
                    ...
                    <div style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
                        <form method="GET" action="dashboard2.php" style="display: flex; gap: 0.5rem;">
                            <input type="text" name="busca" placeholder="Buscar empresa..." value="<?= htmlspecialchars($_GET['busca'] ?? '') ?>" class="search-input" style="padding: 0.5rem 1rem; border: 2px solid var(--border-color); border-radius: 0.5rem; font-family: 'Poppins', sans-serif;">
                            <button type="submit" class="btn btn-primary btn-icon" title="Buscar">
                                <i class="fas fa-search"></i>
                            </button>
                            <?php if (isset($_GET['busca']) && $_GET['busca'] != ''): ?>
                                <a href="dashboard2.php" class="btn btn-secondary btn-icon" title="Limpar busca">
                ...
                <?php if (!empty($busca)): ?>
                    <div style="margin-bottom: 1rem; padding: 1rem; background: #e0f2fe; border-radius: 0.5rem; color: #0369a1;">
                        <i class="fas fa-info-circle"></i> <strong><?= $total ?></strong> resultado(s) encontrado(s) para "<?= htmlspecialchars($busca) ?>"
                    </div>
                <?php endif; ?>
                ...
                <?php if (empty($empresas)): ?>
                    <div class="empty-state">
                        ...
                        <h3><?= !empty($busca) ? 'Nenhuma empresa encontrada' : 'Nenhuma empresa cadastrada' ?></h3>
                        <p><?= !empty($busca) ? 'Tente buscar com outros termos' : 'Comece cadastrando sua primeira empresa' ?></p>
                        <?php if (empty($busca)): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <a href="ver_empresa.php?id=<?= $empresa['id'] ?>" class="btn btn-small btn-success btn-icon" title="Visualizar">
